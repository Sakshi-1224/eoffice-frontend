    ...
    <div className="max-w-4xl mx-auto mt-10 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden animate-fade-in-up">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div
            className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors ${pucFile && pucFile.length > 0 ? "border-teal-400 bg-teal-50" : "border-slate-300 hover:bg-slate-50"}`}
          >
            <UploadCloud
              className={`mx-auto h-10 w-10 mb-2 ${pucFile && pucFile.length > 0 ? "text-teal-600" : "text-slate-400"}`}
            />
          ...
          <div
            className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors ${attachments.length > 0 ? "border-teal-400 bg-teal-50" : "border-slate-300 hover:bg-slate-50"}`}
          >
            <Paperclip
              className={`mx-auto h-10 w-10 mb-2 ${attachments.length > 0 ? "text-teal-600" : "text-slate-400"}`}
            />
              ...
              <div className="mt-4 flex flex-col gap-2">
                ...
                <div className="max-h-32 overflow-y-auto pr-1 space-y-2 custom-scrollbar">
                  {attachments.map((file, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between bg-white p-2 rounded-lg border border-teal-100 shadow-sm text-left group"
                    >
                      <div className="flex items-center gap-2 overflow-hidden">
                        ...
                        />
                        <span
                          className="text-xs text-slate-700 truncate max-w-[120px]"
                          title={file.name}
                        >
                          {file.name}
                        </span>
